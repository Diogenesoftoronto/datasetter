...
class PinterestConfig:
    ...
    def __init__(
        self, search_keywords="", file_lengths=100, image_quality="orig", bookmarks=""
    ):
    ...
    def image_data(self):
        if self.bookmarks == "":
            return (
                '''{"options":{"isPrefetch":false,"query":"'''
                + self.search_keyword
                + """","scope":"pins","no_fetch_context_on_resource":false},"context":{}}"""
            )
        else:
            return (
                '''{"options":{"page_size":25,"query":"'''
                + self.search_keyword
                + '''","scope":"pins","bookmarks":["'''
                + self.bookmark
                + """"],"field_set_key":"unauth_react","no_fetch_context_on_resource":false},"context":{}}""".strip()
            )
